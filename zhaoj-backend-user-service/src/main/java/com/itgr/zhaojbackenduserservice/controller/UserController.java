package com.itgr.zhaojbackenduserservice.controller;
...
import com.itgr.zhaojbackendmodel.model.dto.user.*;
...
public class UserController {
    ...
    public BaseResponse<Long> userRegister(@RequestBody UserRegisterRequest userRegisterRequest) {
        ...
        if (StringUtils.isAnyBlank(userAccount, userName, userPassword, checkPassword)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户名、账户或密码不能为空！");
        }
    ...
    public BaseResponse<Boolean> imageUpdate(@RequestParam("file") MultipartFile multipartFile,
                                             UploadFileRequest uploadFileRequest, HttpServletRequest request) {
        if (multipartFile == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        uploadFileRequest.setBiz(FileUploadBizEnum.USER_AVATAR.getValue());
        String imageURL = fileService.uploadFile(multipartFile, uploadFileRequest);
        User loginUser = userService.getLoginUser(request);
        User user = new User();
